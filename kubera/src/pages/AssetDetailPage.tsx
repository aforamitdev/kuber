...
export function AssetDetailPage() {
  ...
  function addValuation(e: React.FormEvent) {
    ...
    const next: AssetValuation = {
      ...
      value: v,
      mode,
      tolerance: mode === 'approx' && Number.isFinite(t) && t > 0 ? t : undefined,
  ...
  function removeValuation(vid: string) {
    ...
    setAssets(assets.map((a) => (a.id === asset!.id ? { ...a, value: latestVal } : a)))
  }
      ...
      <PageHeader
        ...
        description={
          <span className="inline-flex flex-wrap items-center gap-2">
            ...
            {asset.note && <span className="text-muted-foreground">{asset.note}</span>}
          </span>
        }
      />
        ...
        <form onSubmit={addValuation} className="grid gap-4 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field
              ...
              hint={mode === 'exact' ? 'Disabled for exact' : 'Plus / minus range'}
            >
              <Input
                inputMode="decimal"
                value={tolerance}
                ...
                {[...history].reverse().map((v, idx, arr) => {
                  ...
                  return (
                    <tr key={v.id} className="border-b border-border/60 last:border-b-0">
                      <td className="px-3 py-2 text-xs">{formatDateLong(v.dateISO)}</td>
                      ...
                      <td
                        className={`px-3 py-2 text-right font-mono text-xs ${
                          prev ? (positive ? 'text-emerald-600' : 'text-rose-600') : 'text-muted-foreground'
                        }`}
                      >
                        {prev
